=== WPVacance ===
Contributors: Lucrus
Donate link: https://www.virtualbit.it/wpvacance
Tags: bookings
Requires at least: 4.7
Tested up to: 4.8
Stable tag: trunk
License: GPLv3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Provides shortcodes and admin screens to create a full booking system for accomodations. 

== Description ==

The WPVacance WordPress plugin turns your WordPress site into a booking site for any kind of accomodation. It supports different seasons for different prices and rules based on dates of the booking, any number of accomodation units and standard WordPress users database. Once paired with WooCommerce and Ultimate Member (both are third party products) it makes your site a wonderful ecommerce selling services for holidays or other type of space booking.

Features:

* You can define any number of seasons
* You can define any number of accomodation units
* Sweet javascript enabled booking form: you have never seen anything that easy for end users
* WooCommerce integration
* Integrated cart and Paypal if you don't want WooCommerce
* Compatible with latest WordPress releases
* Free as in free speech
* Free as in free beer
* Community support
* Optional paid support

PRO Features:

* No, there's no PRO version, it's all in the free thing. You can always donate if you want to support us.

Planned features (implementation time depends on received donations):

* Translations
* SEO gimmicks
* anything else donors might come up with

== Installation ==

1. Unzip `wpvacance.zip` into the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress

== Frequently Asked Questions ==

Ask me something frequently enough and I'll be glad to put it here...

== Screenshots ==

1. Here you find the plugin settings
2. Here you see how to configure it

== Changelog ==

= 1.0.3 =
* First public release - Not developed yet. This is not in useable state. Vesion 2.0 will be the first usable release.

== Upgrade Notice ==

= 1.0.3 =
* No notice
